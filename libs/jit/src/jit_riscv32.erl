...
validate_external_call(
    #state{variant = Variant}, {erlang, Function, Arity}
) when
    Variant band ?JIT_VARIANT_MINIMAL_CONCURRENCY =/= 0,
    ((Function =:= spawn andalso Arity =:= 3) orelse
        (Function =:= send andalso Arity =:= 2) orelse
        (Function =:= '!' andalso Arity =:= 2))
->
    ok;
validate_external_call(
    #state{variant = Variant}, {erlang, Function, Arity} = MFA
) when

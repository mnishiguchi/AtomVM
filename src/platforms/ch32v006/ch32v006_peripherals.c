...
static term make_binary_with_roots(Context *ctx, size_t size, uint8_t **data, size_t num_roots, term *roots)
{
    ...
    // The normal 32-bit binary threshold moves buffers of 32 bytes or more to
    // separately allocated refc storage.  These bounded peripheral results are
    ...
    term *boxed = memory_heap_alloc(&ctx->heap, heap_terms);
    boxed[0] = (data_terms << 6) | TERM_BOXED_HEAP_BINARY;
    boxed[1] = size;
...
static term nif_io_binary_probe(Context *ctx, int argc, term argv[])
{
    UNUSED(argv);
    if (argc != 0) {
        return term_invalid_term();
    ...
    uint8_t *data;
    term binary = make_binary(ctx, CH32V006_MAX_IO_BYTES, &data);
    if (term_is_invalid_term(binary)) {
        return binary;
    }
    for (uint8_t i = 0; i < CH32V006_MAX_IO_BYTES; ++i) {
        data[i] = i;
    }
    return binary;
}

static term nif_io_binary_verify(Context *ctx, int argc, term argv[])
{
    UNUSED(ctx);
    if (argc != 1 || !term_is_heap_binary(argv[0])
        || term_binary_size(argv[0]) != CH32V006_MAX_IO_BYTES) {
        return FALSE_ATOM;
    }

    const uint8_t *data = (const uint8_t *) (term_to_const_term_ptr(argv[0]) + 2);
    for (uint8_t i = 0; i < CH32V006_MAX_IO_BYTES; ++i) {
        if (data[i] != i) {
            return FALSE_ATOM;
        }
...
{
#ifdef AVM_CH32V006_IO_BINARY_SELF_TEST
    if (strcmp("ch32v006:io_binary_probe/0", nifname) == 0) {
        return &io_binary_probe_nif;
    }

...
The default support tier is deliberately narrow. It includes:
...
- small-integer `+`, `-`, `*`, `div`, `rem`, and `length`; and
- the documented direct platform NIFs.

Unresolved external calls raise `undef`. Invalid platform-NIF arguments raise
catchable `badarg` errors. Arithmetic accepts only immediate 28-bit signed
integers: an out-of-range result raises catchable `overflow`, while a
non-integer operand or zero divisor raises `badarith`. This keeps unsupported
boxed/big-integer paths out of the firmware.

The default tier does not provide:
...
- boxed and arbitrary-precision integer results;
...
- a 1,536-byte C-stack reserve;
- best-fit allocation;
- free-block coalescing;
- in-place `realloc` shrinking and growth where possible;
- bounded I2C/SPI result binaries stored directly on the process heap;
...
I2C and SPI results are limited to 64 bytes. Their platform helper deliberately
uses heap binaries throughout that range instead of AtomVM's normal 32-bit
threshold, which moves 32-byte and larger values to separately allocated
ref-counted storage. The bounded representation reduces code and allocator
overhead while retaining ordinary binary term semantics.
...
Most acceptance images shorten the tracking interval to 10 ms. The dedicated
production-timing soak keeps the normal interval, crosses two natural counter
wraps, and uses only readback instrumentation; it does not reposition SysTick.
...
The default language/runtime and GPIO workloads remain separate. The opt-in
timer/concurrency tier has its own combined GPIO application so its exact
configuration and resource margin can be qualified independently.
...
Optional runtime profiles use native-image variant bits for concurrency, timers,
and byte binaries, so firmware cannot silently load code compiled for an absent
...
profile. It uses a 2,048-byte C-stack reserve and supports same-module
`spawn/3`, local-PID `send/2`, BEAM send/receive operations, selective receive,
and receive timeouts. The explicit `send/2` import is required by Elixir, while
Erlang commonly emits the send opcode directly. Both paths use the same
constrained local-process semantics; linked spawning, registered names, ports,
...
electrical qualification remains open. Their small direct-NIF APIs are
platform experiments rather than compatibility claims
